// Mesh Text Hover — ported from Originkit (Framer) to plain JS/React.
// Renders text into a WebGL2 mesh that deforms as you drag the cursor over it,
// with an optional RGB-split chroma effect.
...
// Parse a CSS colour string into a [0..1, 0..1, 0..1] tuple. Falls back to white.
...
export default function MeshText(userProps) {
  const props = { ...COMPONENT_DEFAULTS, ...userProps };
  const { text, color, font, colorSplit, customColors, force } = props;

  // Live refs so toggles/sliders take effect mid-loop without rebuilding GL state.
